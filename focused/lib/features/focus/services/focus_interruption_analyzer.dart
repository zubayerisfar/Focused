...
class FocusInterruptionAnalyzer {
  FocusAnalysisResult analyzeSession({
    ...
    final focusRanges = _mergeRanges(
      session.focusIntervals.map(
        (interval) =>
            _TimeRange(start: interval.startTime, end: interval.endTime),
      ),
    );

    final actualFocusDuration = _sumRanges(focusRanges);
    ...
    final Map<String, List<_TimeRange>> distractionRangesByApp = {};
    ...
    for (final record in usageRecords) {
      ...
      appIds.putIfAbsent(record.appName, () => record.appId);

      for (final focusRange in focusRanges) {
        final intersection = _intersection(
          _TimeRange(start: record.startTime, end: record.endTime),
          focusRange,
        );
    ...
    for (final entry in distractionRangesByApp.entries) {
      final appName = entry.key;

      final mergedRanges = _mergeRanges(entry.value);

      distractionByApp[appName] = _sumRanges(mergedRanges);
    ...
    final distractionEpisodes = _mergeRanges(
      interruptions.map(
    ...
    final distractedDuration = _sumRanges(distractionEpisodes);
    ...
    final effectiveSeconds =
        (actualFocusDuration.inSeconds - distractedDuration.inSeconds).clamp(
          0,
    ...
    final plannedSeconds = session.plannedFocusDuration.inSeconds;

    final actualSeconds = actualFocusDuration.inSeconds;

    final effective = effectiveFocusDuration.inSeconds;

    final completionRate = plannedSeconds <= 0
        ? 0.0
        : ((actualSeconds / plannedSeconds) * 100).clamp(0.0, 100.0).toDouble();

    final attentionRetention = actualSeconds <= 0
        ? 0.0
        : ((effective / actualSeconds) * 100).clamp(0.0, 100.0).toDouble();

    final focusQuality = actualSeconds <= 0
        ? 100.0
        : ((effective / actualSeconds) * 100).clamp(0.0, 100.0).toDouble();
    ...
    return FocusAnalysisResult(
      ...
      interruptionCount: distractionEpisodes.length,
  ...
  List<_TimeRange> _mergeRanges(Iterable<_TimeRange> ranges) {
    ...
    for (int i = 1; i < validRanges.length; i++) {
      final next = validRanges[i];

      // Overlapping OR directly touching ranges
      // are one continuous interval.
      if (!next.start.isAfter(current.end)) {
        final laterEnd = next.end.isAfter(current.end) ? next.end : current.end;

        current = _TimeRange(start: current.start, end: laterEnd);
      } else {
        merged.add(current);
  ...
  Duration _sumRanges(Iterable<_TimeRange> ranges) {
    ...
    for (final range in ranges) {
      totalMilliseconds += range.end.difference(range.start).inMilliseconds;
    }
